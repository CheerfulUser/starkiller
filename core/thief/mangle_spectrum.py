...
def mangle_spectrum2(wave,flux,bands, mags, fixed_filters=None, 
   ...
   Args:
      ...
      flux (float array):  Input fluxes in arbitrary units (flam scale)
   ...
   if (type(bands) == type(OrderedDict())) | (type(bands) == dict):
      names = list(bands.keys())
      for name in names:
         wav = bands[name][0].wave
         through = bands[name][0].throughput
         fset[name] = filter(name)
         fset[name].wave = wav
         fset[name].resp = through
         fset[name].zp = bands[name][1]
         fset[name].zp = 16.84692 + 2.5*num.log10(
                           scipy.integrate.trapz(fset[name].resp/fset[name].wave, x=fset[name].wave))
      bands = names 
